...
Usage (from la-proteina-main/ directory):
    DATA_PATH=./data python run_baseline_task1.py --noise_scale 0.1 --num_pairs 50
"""
...
# Project root is current working directory (script must be run from la-proteina-main/)
...
def patch_full_simulation(model, noise_scale):
    ...
    def patched_full_sim(batch, *args, **kwargs):
        # Pass noise_scale to the fixed full_simulation
        kwargs["init_noise_scale"] = noise_scale
        # Disable path mixing and MLP
...
def main():
    ...
    # Load config using Hydra with ABSOLUTE path
